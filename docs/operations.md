...
`beacon-trace-incident-grid` is centered on observability work in Scala. The technical target is: Package a Scala local lab for incident analysis with round-trip fixtures, lossless normalization checks, and documented operating limits.
...
The score is deterministic. The current threshold is 184. Risk has a penalty of 6, latency has a penalty of 4, and weight contributes a bonus of 3.
